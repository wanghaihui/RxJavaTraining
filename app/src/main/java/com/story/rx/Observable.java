...
public class Observable<T> {
     ...
     * OnSubscribe 会被存储在返回的 Observable 对象中 -- onSubscribe
     * 相当于一个计划表，当 Observable 被订阅的时候，OnSubscribe 的 call() 方法会自动被调用，事件序列就会依照设定依次触发
     *
     ...
     *
     * 当Observable.subscribe被调用时，才会触发
     */
     ...
     * 使用 compose() 方法，Observable 可以利用传入的 Transformer 对象的 call 方法直接对自身进行处理，也就不必被包在方法的里面
     *
     ...
     * Observer subscribes to the Observable. This allows an {@link java.util.Observer} to easily obtain updates or a
     ...
     *            the Observable factory function to invoke for each {@link java.util.Observer} that subscribes to the
     *            resulting Observable
     * @param <T>
     *            the type of the items emitted by the Observable
     * @return an Observable whose {@link java.util.Observer}s' subscriptions trigger an invocation of the given
    ...
    /**
     * Returns an Observable that emits no items to the {@link java.util.Observer} and immediately invokes its
     * {@link java.util.Observer#onCompleted onCompleted} method.
     * <p>
     ...
     * @return an Observable that emits no items to the {@link java.util.Observer} but immediately invokes the
     *         {@link java.util.Observer}'s {@link java.util.Observer#onCompleted() onCompleted} method
    ...
    /**
     * Returns an Observable that invokes an {@link java.util.Observer}'s {@link java.util.Observer#onError onError} method when the
     ...
     * 
     * @param exception
     *            the particular Throwable to pass to {@link java.util.Observer#onError onError}
     * @param <T>
     *            the type of the items (ostensibly) emitted by the Observable
     * @return an Observable that invokes the {@link java.util.Observer}'s {@link java.util.Observer#onError onError} method when
     ...
     * @return an Observable whose {@link java.util.Observer}s' subscriptions trigger an invocation of the given function
     * @see #defer(Func0)
     * @since (if this graduates from Experimental/Beta to supported, replace this parenthetical with the release number)
     */
    @Experimental
     ...
     * Observables notify of an error via {@link java.util.Observer#onError onError}, {@code mergeDelayError} will refrain
     ...
     * the merged Observables notify of an error via {@link java.util.Observer#onError onError}, {@code mergeDelayError}
     ...
     * if any of the merged Observables notify of an error via {@link java.util.Observer#onError onError},
     ...
     * except that if any of the merged Observables notify of an error via {@link java.util.Observer#onError onError},
     * {@code mergeDelayError} will refrain from propagating that error notification until all of the merged
     * Observables have finished emitting items.
     * <p>
     * <img width="640" height="380" src="https://raw.github.com/wiki/ReactiveX/RxJava/images/rx-operators/mergeDelayError.png" alt="">
    ...
    /**
     * Returns an Observable that never sends any items or notifications to an {@link java.util.Observer}.
     * <p>
     ...
     * @return an Observable that never emits any items or sends any notifications to an {@link java.util.Observer}
    ...
    public final Observable<T> doOnCompleted(final Action0 onCompleted) {
        java.util.Observer<T> observer = new java.util.Observer<T>() {
            @Override
     ...
     * @see <a href="http://reactivex.io/documentation/operators/do.html">ReactiveX operators documentation: Do</a>
     */
    public final Observable<T> doOnEach(java.util.Observer<? super T> observer) {
        return lift(new OperatorDoOnEach<T>(observer));
    }
     ...
     * {@link java.util.Observer#onCompleted onCompleted} or {@link java.util.Observer#onError onError}.
     * <p>
     ...
     * <p>
     * This method returns an Observable that will invoke a subscribing {@link java.util.Observer}'s
     * {@link java.util.Observer#onNext onNext} function a maximum of {@code count} times before invoking
     * {@link java.util.Observer#onCompleted onCompleted}.
     * <dl>
     *  <dt><b>Scheduler:</b></dt>
    ...
    /**
     * 事件对象的直接变换，具体功能上面已经介绍过--是 RxJava 最常用的变换
     *
     ...
     * 
     * @param scheduler
     *            the {@link Scheduler} to notify {@link java.util.Observer}s on
     * @return the source Observable modified so that its {@link java.util.Observer}s are notified on the specified
     *         {@link Scheduler}
     ...
     * {@link java.util.Observer#onError onError} if it encounters an error.
     * <p>
     ...
     * its {@link java.util.Observer}, the Observable invokes its Observer's {@code onError} method, and then quits
     ...
     * {@code resumeFunction}, which will invoke the Observer's {@link java.util.Observer#onNext onNext} method if it is
     ...
     * will invoke the Observer's {@link java.util.Observer#onNext onNext} method if it is able to do so. In such a case,
    ...
    /**
     * Instructs an Observable to pass control to another Observable rather than invoking
     * {@link java.util.Observer#onError onError} if it encounters an {@link Exception}.
     * <p>
     ...
     * to its {@link java.util.Observer}, the Observable invokes its Observer's {@code onError} method, and then quits
     ...
     * {@code resumeSequence} which will invoke the Observer's {@link java.util.Observer#onNext onNext} method if it is
     * able to do so. In such a case, because no Observable necessarily invokes {@code onError}, the Observer
     ...
     *         to its {@link java.util.Observer}s
     ...
     *         items to its {@link java.util.Observer}s
     * @see <a href="http://reactivex.io/documentation/operators/replay.html">ReactiveX operators documentation: Replay</a>
     */
    ...
    /**
     * Returns a {@link ConnectableObservable} that shares a single subscription to the source Observable that
     * will replay all of its items and notifications to any future {@link java.util.Observer} on the given
     ...
     * <p>
     * If the source Observable calls {@link java.util.Observer#onError}, this method will resubscribe to the source
     ...
     * </dl>
     * 
     * @return a {@link Subscription} reference with which the {@link java.util.Observer} can stop receiving items before
     ...
     * @return
     */
    public final Subscription subscribe(final java.util.Observer<? super T> observer) {
     ...
     * @return a {@link Subscription} reference with which Subscribers that are {@link java.util.Observer}s can
     ...
     * <p>
     * Normally, an Observable that returns multiple items will do so by invoking its {@link java.util.Observer}'s
     * {@link java.util.Observer#onNext onNext} method for each such item. You can change this behavior, instructing the
    ...
    /**
     * An Observable that never sends any information to an {@link java.util.Observer}.
     * This Observable is useful primarily for testing purposes.
     * 
     * @param <T>
    ...
    /**
     * An Observable that invokes {@link java.util.Observer#onError onError} when the {@link java.util.Observer} subscribes to it.
     * 
     * @param <T>
    ...
    private static class ThrowObservable<T> extends Observable<T> {
        ...
        public ThrowObservable(final Throwable exception) {
            super(new OnSubscribe<T>() {
                ...
                /**
                 * Accepts an {@link java.util.Observer} and calls its {@link java.util.Observer#onError onError} method.
                 * 
                 * @param observer
